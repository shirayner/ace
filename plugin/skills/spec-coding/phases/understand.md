...
| 级别     | 判定条件                         | 处理方式                |
| -------- | -------------------------------- | ----------------------- |
| 必须澄清 | 假设失败成本高 + 不可推断        | 向用户提问              |
| 记录假设 | 假设失败成本低，或可从上下文推断 | AI 做出假设，对齐时展示 |
...
**关键假设**
- 假设 1（来自上下文中 status=assumed 的条目）
- 假设 2
...
<HARD-GATE>
四要素缺一不可。"完成标准"必须是从需求直接推导出的可测试条件（能用来验收），不是泛泛的"功能可用"。缺失完成标准 = 未完成 Step 10，不得进入审批。
</HARD-GATE>
...
通过自检 → 进入 Step 11 审批。未通过 → 回到 Step 9。
