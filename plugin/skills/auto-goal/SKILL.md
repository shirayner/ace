...
**Step 1: 分析** — 域判断 + 对齐评估 + 深度估算

**Step 2: 澄清（按需）** — 分析中发现 Must-Ask/Should-Ask 触发？
→ 调用 Clarify 向用户提问，获得答案后更新理解
→ 无触发则跳过

**Step 3: 呈现对齐** — 向用户简明呈现，等待确认：
1. **我的理解** — 用一两句话复述目标的核心意图
2. **计划方向** — 打算怎么做（高层策略，不是细节）
3. **关键假设** — 标注但置信度足够的假设（如果有）

**用户确认后**才进入 OODA 循环。用户纠正 → 更新理解 → 可能需要再次澄清和对齐。

```
接收目标 → 初步分析（域判断 + 对齐评估 + 深度估算）
  ↓
有歧义？──→ ⚡Clarify（交互式提问）→ 更新理解
  ↓
⚡首轮对齐呈现（MANDATORY，此时理解已充分）
  理解 + 方向 + 假设 → 等待确认
  用户确认 → 进入执行 ┃ 用户纠正 → 更新理解 → 可能再次 Clarify
  ↓
```
...
消除歧义，对齐理解。区分表面需求与真实需求，检验问题框架是否最优。**任何阶段**发现不确定性时可调用——初步分析阶段、首轮对齐前、OODA 循环中均适用。

**执行方式**：通过平台交互工具向用户提问（参见 interactive-clarify 规则）。

**核心原则：技术实现细节能推断就自己做；方向、偏好、取舍必须确认。**

**Must-Ask 触发器（硬性，命中任一必须提问）**：
