...
description: 为代码仓库生成 LLM Wiki 知识库,供需求评审/技术方案 Agent 渐进式加载。
  支持后端(api/mq/job)和前端(page/component)入口。
  当用户说"生成项目 Wiki"、"更新项目知识库"、"构建 LLM 文档"、"为这个仓库做 Wiki"时触发。
  入口由项目根目录 .ace/wiki/_meta.yml 配置驱动,通过对话调整状态。
---
...
- "自动扫描入口" / "扫描入口" (在已有 _meta.yml 但未配置时)
- "开始构建" / "继续构建" (在配置就绪后)
...
```
已初始化 .ace/wiki/_meta.yml。入口配置为空,请选择:

1. 我来手动编辑 .ace/wiki/_meta.yml(参考文件内注释示例)
2. 自动扫描全仓库(用内置规则识别所有入口,写回 _meta.yml 后请你确认)
3. 用自然语言告诉我扫描范围
...
流程:
1. 读取 `.ace/wiki/_meta.yml`
2. 解析 `entries` 字段(见"入口解析"章节)
3. 进入四阶段并行构建流水线(见"构建流水线"章节)

## 入口解析

从 `.ace/wiki/_meta.yml` 的 `entries` 字段解析入口列表。
...
解析 _meta.yml 获得入口清单后,执行四阶段流水线:

```
Phase 1: 入口发现(串行)
  → Phase 2: 入口 Wiki 生成(并行,独立)
...
### Phase 2:入口 Wiki 生成(并行)

**设计约束:入口独立生成,不共享状态,不依赖缓存。**

对每个入口并发执行(上限 min(入口数, 8)个并行子任务):
...
**失败处理**:单入口失败不阻塞其他,降级标记 failed 继续。
...
2. 收集所有入口 frontmatter 的 `description + related_business + business_scenario`
3. LLM 提炼并填充:
   - **frontmatter 元信息**:从 _meta.yml 或代码推断 `business_domain`/`business_subdomain`/`project_type`/`keywords`
   - **快速查找**(5-8 条):"我想了解XX" → 入口,帮助 Agent 快速定位
   - **核心业务流程**(3-5 条):每条 1 行概要 + 结构化入口列表(标注各入口在流程中的角色)
   - **核心领域模型**(5-8 个):只纳入业务领域概念,排除技术实现模式;每个模型 1 句定义 + 关联入口(标注关联原因)
...
```
=== LLM Wiki 构建报告 ===
入口:api 12 / mq 1 / job 1 (共 14)

成功:14  失败:0

Token 消耗:
  入口生成: input 85,432 | output 124,678
...
| `templates/api.md` | API 入口 Wiki 模板 | Phase 2 生成 api 入口时 |
| `templates/mq.md` | MQ 入口 Wiki 模板 | Phase 2 生成 mq 入口时 |
| `templates/job.md` | Job 入口 Wiki 模板 | Phase 2 生成 job 入口时 |
| `templates/page.md` | 页面入口 Wiki 模板 | Phase 2 生成 page 入口时 |
| `templates/component.md` | 组件入口 Wiki 模板 | Phase 2 生成 component 入口时 |
...
| 单入口 Wiki ~2000-5000 tokens | 典型范围 |
| INDEX.md ~800 tokens | 20 个入口项目 |
| SUMMARY.md ~1500 tokens | 15 个入口项目 |
