...
def run_batch(backend, batch_file):
    """
    批量模式：从 JSON 文件读取任务列表，脚本内部线程池并行执行。
    ...
    def execute_task(idx, task):
        ...
        if not prompt and "prompt_file" in task:
            with open(task["prompt_file"], "r", encoding="utf-8") as fp:
        ...
        result = generate_and_save(
            backend,
            prompt=prompt,
            output_path=task.get("output", f"output_{idx}.png"),
...
def execute(backend, args):
    """根据已解析的参数执行单张或批量生成（构造好 backend 后调用）。"""
    if args.batch:
        run_batch(backend, args.batch)
        sys.exit(0)
