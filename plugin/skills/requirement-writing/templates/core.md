...
**北极星指标**：<仅在模型明确提供时填写；详细口径见“成功信号与指标”>
...
> 定性和部分量化 success signal 都按 confirmed 模型内容原样写成叙述或列表，不擅自补齐。只有基线、目标、测量口径和时间窗全部明确时才出现结构化量化指标表。缺少四要素本身不产生 ProjectionGap；仅当模型将指标定义为验收或发布门槛，且业务上因此无法判定时才返回 ProjectionGap。

```markdown
## 成功信号与指标

**成功信号**（模型提供时）
- <定性信号，或按已确认内容原样描述的部分量化信号>
...
> 每个 REQ 必须有至少一种可验证完成条件。以下验收格式按需求类型选择或组合，删除未选格式；不强制 Given/When/Then，也不得据此创建新的 canonical 模型对象。
...
**验收标准：Given/When/Then（条件分支、异常或边界）**
| 编号 | 场景 | Given（前置条件） | When（动作 / 事件） | Then（可验证结果） |
|------|------|-------------------|---------------------|--------------------|
| AC-002 | <场景名> | <前置条件> | <用户动作 / 系统事件> | <可验证结果> |
...
**验收标准：状态转换表（状态需求）**
| 编号 | 当前状态 | 事件 / 条件 | 下一状态 | 可观察结果 |
|------|----------|-------------|----------|------------|
| AC-004 | <状态> | <事件 / 条件> | <状态> | <可验证结果> |

**验收标准：口径 / 指标阈值（数据或质量属性）**
| 编号 | 验收对象 | 测量口径 | 通过阈值 |
|------|----------|----------|----------|
| AC-005 | <数据 / 质量属性> | <统计或测量方式> | <可判定阈值> |
````
