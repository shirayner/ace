...
- 四步是思考顺序，不是四份产物。单次直写模式可合并；默认只交付最终 PRD，不展示中间态。
- **Plan 是价值所在**：动笔前决定整份 PRD 如何组织，避免重复与结构漂移。规模不硬触发章节；但正文规模超出一次稳定生成能力时，必须切换到下面的分阶段执行模式。

### 投影执行模式

<HARD-GATE name="执行模式不改写需求规模">
执行模式只决定「这次怎么写」，**不是需求规模**。文档信息里的 `需求规模` 是上游需求理解阶段的权威结论（取值只有 `Micro` / `Normal`），必须原样投影，禁止在写作阶段依据文档长度、章节数或 REQ 个数重新推断（见 projection-rules R-S8）。执行模式不写入 PRD 正文。
</HARD-GATE>

| 模式 | 何时用 | 怎么做 |
|------|--------|--------|
| **单次直写** | 正文可在一次生成中稳定完成 | 直接执行四步；可先列内部章清单，不强制创建临时状态文件 |
| **分阶段落盘** | 命中任一强信号 | 禁止在一个生成回合里规划并写完整篇正文；先外化计划和骨架，再按依赖逐章落盘 |

**分阶段强信号**（命中任一即切换）：主材料需要分块读取；多份来源文档共同决定需求；跨多个业务域且含多组 REQ / BR / Extension；预计正文无法在一次稳定生成中完成。上游规模为 `Normal` 只是提示，不自动触发；`Micro` 也可能因来源分散而命中。

### 分阶段落盘（Freeze → Scaffold → Fill → Check）

<HARD-GATE name="分阶段先骨架后正文">
...
| **Fill** | 按 chapter-tree 的分阶段编写依赖顺序，每次只写一个章节；功能需求每次只写一个 REQ + 其 AC；写入时替换该章 working 标记 | 章节内容已写入目标 PRD，state 中该章标为 done |
...
6. **结构化流程出图且图文一致**（R-T14 / R-T15）——主流程命中分支 / 过滤 / 循环 / 并行 / 回流 / 跨角色或系统交接 / 多阶段跳转任一特征时必须给 Mermaid 图，仅单角色线性短流程可豁免且须显式声明；有图则节点 / 条件 / 去向须与主流程步骤、异常分支表及所引 BR / REQ 语义一致。
...
- 范围清单 / Plan / 自检记录均为内部工作态，不默认展示、不要求确认。Coverage 映射**必须建**（P0 保真的强制证据，见 projection-rules 自检 P0）；分阶段落盘将其持久化在 projection-state，最终交付前删除。
