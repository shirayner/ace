...
上面的章节树是**最终展示顺序**，不是生成顺序。分阶段落盘模式（见 SKILL.md「投影执行模式」；与 PRD 里的需求规模无关）必须按内容依赖生成：先冻结上游结论，再写依赖它的章节；TL;DR 虽展示在顶部，但最后编写。
...
4. **Flow**：存在端到端时序，或命中 R-T14 任一结构化特征时编写；命中任一结构化特征的，本章必须含 Mermaid 图。仅无端到端时序且七项特征全不命中时可跳过。
...
- 无端到端时序，且 R-T14 七项结构化特征全不命中 → 跳过 Business Flow，REQ 直接依赖 Why / Who / 局部规则。命中任一特征则本章必出，且必须带图——不得以"功能简单"为由跳章来规避出图。
