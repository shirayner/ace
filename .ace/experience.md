...
### E6: 共享存储改写前先完成目标投影冲突预检
- **场景**：一次安装既重建共享 canonical store，又向受保护的目标目录投影副本。
- **发现**：未托管同名目录的冲突必须在重建 canonical store 前汇总检查；若在逐目标投影时才失败，会留下“共享存储已更新、目标副本未更新”的部分安装状态。预检需要结合上一份回执区分 ACE 托管路径与第三方路径。
- **适用范围**：包含共享源存储和多个目标投影、且要求保护目标现有内容的安装器。
- **验证记录**：首次记录（2026-08-30）
