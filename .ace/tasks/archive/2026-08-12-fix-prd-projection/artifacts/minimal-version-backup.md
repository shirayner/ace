...
**口径说明（R-T11）**：本章「30 天窗口」是**入口露出过滤口径**（行程开始日期 ≤ 访问当日 + 30 天，做成配置项）；背景测算引用的「未来 60 天有待出行订单」是**覆盖面测算口径**。两者不是同一口径，不可互相替代。
...
4. 逐条候选行程判断四类权益可用性（条件见 2.2）→ 标记每条行程可用的权益类型。自定义行程（仅支持自定义机票、酒店、火车；无订单号，但有产线与特殊标识）不推休息室，其余三类权益照常判断。
