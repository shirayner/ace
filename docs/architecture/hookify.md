...
| `git reset --hard` | 丢弃未提交更改 | 先用 `git stash` 保存 |
| `git clean -fd` | 删除未跟踪文件 | 先用 `-n` 预览 |
| `git rebase -i` | 改变提交历史 | 仅对本地提交使用 |
| `git commit --amend` | 修改已推送提交 | 仅修改本地最新提交 |

### 用户交互

```
⚠️  即将执行强制推送 (git push --force)

这会覆盖远程分支历史，可能影响其他协作者
建议先用 git push --force-with-lease
...
| 检查项 | 说明 | 处理方式 |
|--------|------|---------|
| 调试代码 | `console.log`, `debugger;` | 警告移除 |
| 代码规范 | 函数长度、嵌套深度 | 警告优化 |
| 敏感信息 | 硬编码密码、API Key | 严重警告 |
| TODO/FIXME | 待办事项标记 | 提醒跟进 |
